...
template<class Proto> 
bool safeSerializeToString(Proto& proto, std::string& protoString) {
	try{
		return proto.SerializeToString(&protoString);
	}
	catch(std::exception&) {
		return false;
	}
...
template<class Proto>
bool safeParseFromString(Proto& proto, const std::string& protoString) {
	try{
		return proto.ParseFromString(protoString);
	}
	catch(std::exception&) {
		return false;
	}
...
bool readProtoFromStream(Proto& proto, std::istream& stream) {
	...
	return readStringWithSizeFromStream(protoString, stream) && safeParseFromString(proto, protoString);
}

template<class Proto>
bool writeProtoOnStream(const Proto& proto, std::ostream& stream) {
	std::string protoString;
	return safeSerializeToString(proto, protoString) && writeStringWithSizeOnStream(protoString, stream);
}
