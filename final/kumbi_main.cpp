...
int main()
{
    ...
    do
    {
        char i;
        cin>>i;
        if(van_bomba()) move_doc(shortest_path('e'));
        else move_doc(shortest_path('l'));
        show_tick();
        compute();
        draw_table('r');
        draw_table('h');
                ...
                if(table[j][i]==2)
                {
                    ...
                    if(health_table[j][i]==0)
                    {
                        ...
                        do
                        {
                            sugar++;
                            int a;
                            if((table[j][i-sugar]==0 || table[j][i-sugar]==-3) && iranyok[0]==0) a++;
                            else
                            {
                                iranyok[0]=1; kesz++;
                                if(table[j][i-sugar]==1) health_table[j][i-sugar]--;
                            }
                            if((table[j][i+sugar]==0 || table[j][i+sugar]==-3) && iranyok[1]==0) a++;
                            else
                            {
                                iranyok[1]=1; kesz++;
                                if(table[j][i+sugar]==1) health_table[j][i+sugar]--;
                            }
                            if((table[j-sugar][i]==0 || table[j-sugar][i]==-3) && iranyok[2]==0) a++;
                            else
                            {
                                iranyok[2]=1; kesz++;
                                if(table[j-sugar][i]==1) health_table[j-sugar][i]--;
                            }
                            if((table[j+sugar][i]==0 || table[j+sugar][i]==-3) && iranyok[4]==0) a++;
                            else
                            {
                                iranyok[3]=1; kesz++;
                                if(table[j+sugar][i]==1) health_table[j+sugar][i]--;
...
void move_doc(char c)
{
    switch(c)
    {
        case 'u': if(table[doc_x][doc_y-1]==0 || table[doc_x][doc_y-1]==-2 || path_table[doc_x][doc_y-1]==2)
        {
            if(path_table[doc_x][doc_y-1]==2)
            {
                if(can_escape('u')) put_bomb('u',activating_time,2); tick--;
            }
            else
            {
                table[doc_x][doc_y]=0;
        ...
        case 'd': if(table[doc_x][doc_y+1]==0 || table[doc_x][doc_y+1]==-2 || path_table[doc_x][doc_y+1]==2)
        {
            if(path_table[doc_x][doc_y+1]==2)
            {
                if(can_escape('d')) put_bomb('d',activating_time,2); tick--;
            }
            else
            {
                table[doc_x][doc_y]=0;
        ...
        case 'l': if(table[doc_x-1][doc_y]==0 || table[doc_x-1][doc_y]==-2 || path_table[doc_x-1][doc_y]==2)
        {
            if(path_table[doc_x-1][doc_y]==2)
            {
                if(can_escape('l')) put_bomb('l',activating_time,2); tick--;
            }
            else
            {
                table[doc_x][doc_y]=0;
        ...
        case 'r': if(table[doc_x+1][doc_y]==0 || table[doc_x+1][doc_y]==-2 || path_table[doc_x+1][doc_y-1]==2)
        {
            if(path_table[doc_x-1][doc_y]==2)
            {
                if(can_escape('r')) put_bomb('r',activating_time,2); tick--;
            }
            else
            {
                table[doc_x][doc_y]=0;
...
int can_escape(char c)
{
    ...
    switch(c)
    {
        case 'u': temp_table[doc_x][doc_y-1]=2;
        case 'd': temp_table[doc_x][doc_y+1]=2;
        case 'l': temp_table[doc_x-1][doc_y]=2;
        case 'r': temp_table[doc_x+1][doc_y]=2;
    }
